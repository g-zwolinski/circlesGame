...
module.exports = function(passport) {
    ...
    function(req, email, password, done) {
        ...
        process.nextTick(function() {
            var sprawdzCzyIstniejeMailINazwa = 0;

            if (!req.user) {
                User.findOne({ 'user.email' :  email }, function(err, user) {
            	    if (err){ return done(err);}
                    if (user) {
                        return done(null, false, req.flash('signuperror', 'Email already exists in database.'));
                    } else {
                        sprawdzCzyIstniejeMailINazwa++;
                        console.log(sprawdzCzyIstniejeMailINazwa);
                        console.log(req.body.username);
                        var nameToCheck = req.body.username;
                        User.findOne({ 'user.username' :  nameToCheck }, function(err, user) {
                            if (err){ return done(err);}
                            if (user) {
                                return done(null, false, req.flash('signuperror', 'Username already exists in database.'));
                            } else {
                                sprawdzCzyIstniejeMailINazwa++;
                                if(sprawdzCzyIstniejeMailINazwa==2){
